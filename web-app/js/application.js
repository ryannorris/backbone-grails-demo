...
// fix the _.template method so that processing instructions don't conflict with GSP
_.templateSettings = {
	interpolate : /\{\{(.+?)\}\}/g
};
...
Medullan.Router = (function() {
	...
	var friendsRouter = Backbone.Router.extend({
		...
		/*
		 * I'm instantiating my parent views in the constructor, as well as the colections/models
		 * that manage the state of the application
		 */
		initialize: function() {
        	this.friends = new Medullan.Model.People();
        	this.friendList = new Medullan.View.FriendListView({ collection: this.friends });
        	this.friendList.hide();
        	
        	this.selectedFriend = new Medullan.Model.Person();
        	this.friendDetailView = new Medullan.View.FriendDetailView({ model: this.selectedFriend });
		...
		index: function() {
			...
			// when coming into this state, hide the detail view and show the list view
			this.friendDetailView.hide();
			this.friendList.show();
			
        	var self = this;
        	
        	// when a person is selected in the person list, show that person's information
        	this.friendList.bind("personSelected", function(e) {
        		self.navigate("friend/" + e.person.id, true);
        	});
        	
        	// load the person data from the server, typically done from a bootstrap
        	this.friends.fetch();
		},
		...
		friendDetail: function(friendId) {
			...
			// the re-rendering of the model.  We also save ourselves a roundrtip by loading
			// right from the current client state
			this.selectedFriend.set(this.friends.get(friendId).toJSON());
			
			// just like with "/", now toggle to hide the list and show the detail
			this.friendList.hide();
...
Medullan.Model = (function() {
	...
	var people = Backbone.Collection.extend({
		...
		// we need to implement this due to impedence issues between grails and backbone 
...
Medullan.View = (function() {
	...
	// basicControl is a base class, and is effectively no instantiable from outside the Medullan.View
	// namespace
	...
	var nameView = basicControl.extend({
		...
		initialize : function() {
			_.bindAll(this, "render", "name_onClick", "name_onMouseover", "name_onMouseout");
		...
		render : function() {
			...
			// i always return "this" from render so that if the state of the render
			// is managed internal to the object, i can always get this from the "el"
			// property
			return this;
		},
		
		name_onClick: function() {
			this.trigger("clicked", { data: this.model });
		},
	...
	var friendDetail = basicControl.extend({
		...
		initialize: function() {
			...
			// when the underlying model changes (e.g. old.set(new.toJSON())) we re-render
			this.model.bind("change", this.render);
		},
	...
	var friendList = basicControl
			.extend({
				...
				initialize : function() {
					_.bindAll(this, "render", "button_onClick", "addFriend", "renderOne", "nameClicked");
					_.extend(this, Backbone.Events);
					
					// we bind two methods to the collection - one to deal with the addition of ONE friend
				...
				render : function() {
					...
					// when the entire collection changes, we re-render ALL of the models in the backing collection
				...
				renderOne: function(person) {	// render one person at a time
					...
					// when the name is clicked, fire an event so parent views (this view)
				...
				nameClicked: function(e) {
					this.trigger("personSelected", { person: e.data });
				},
